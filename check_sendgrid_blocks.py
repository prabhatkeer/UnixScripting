...
##----------------------------------------------------------------------------------------------------
## This script uses the Sendgrid V2 API to gather the count for the blocked emails for the past one day
...
try:
    ...
    count_blocked = int(json_data['count'])

    if count_blocked >= 50:
        print "[CRITICAL] " + str(count_blocked) + " blocked emails since yesterday"
        exit(2)

    elif count_blocked >= 10:
        print "[WARNING] " + str(count_blocked) + " blocked emails since yesterday"
        exit(1)

    else:
        print "[OK] " + str(count_blocked) + " blocked emails since yesterday"
        exit(0)
