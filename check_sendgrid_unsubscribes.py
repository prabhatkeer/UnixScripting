...
##----------------------------------------------------------------------------------------------------
## This script uses the Sendgrid V2 API to gather the count for the unsubscribes for the past one day
## https://sendgrid.com/docs/API_Reference/Web_API/unsubscribes.html
##
## CHANGE LOG:
##----------------------------------------------------------------------------------------------------
...
try:
    ...
    f = urllib.urlopen("https://api.sendgrid.com/api/unsubscribes.count.json?%s" % params)
    response = f.read()
    json_data = json.loads(response)
    count_unsubscribes = int(json_data['count'])

    if count_unsubscribes >= 50:
        print "[CRITICAL] " + str(count_unsubscribes) + " unsubscribed since yesterday"
        exit(2)

    elif count_unsubscribes >= 10:
        print "[WARNING] " + str(count_unsubscribes) + " unsubscribed since yesterday"
        exit(1)

    else:
        print "[OK] " + str(count_unsubscribes) + " unsubscribed since yesterday"
        exit(0)
