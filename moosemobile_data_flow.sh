...
put_logistics_to_ftp() {
  ...
  for file in `ls *.csv`; do
    if lftp -e "set ftp:passive-mode false; set ssl:verify-certificate no; lcd $local_folder; cd $remote_folder; put $file; bye" -u $ftpUser,$ftpPass $ftpHost; then
      ...
      logger -p local6.notice -s -t $(basename $0) "[ERROR] Logistics file $file for moosemobile upload to ftp FAILED
 for some reason";
    fi
  done;
}
...
get_files_from_ftp() {
  ...
  if lftp -e "set ftp:passive-mode false; set ssl:verify-certificate no; set xfer:clobber on; lcd $local_folder; cd $remote_folder; mget *.csv; renlist > downloaded_list.lst; put -c -E downloaded_list.lst; bye" -u $ftpUser,$ftpPass $ftpHost; then
    logger -p local6.notice -s -t $(basename $0) "[OK] Files shipped by customer for moosemobile downloaded from ftpserver"
  else
...
move_cdrs_to_processed (){
  in_dir=/shared/application_data/owi_084
  out_dir=/shared/application_data/moosemobile/cdr_unprocessed
  
  if [ -e $in_dir/RESELL*.zip ]; then
      logger -p local6.notice -s -t $(basename $0) "[OK] RESELL files are moved from $in_dir to $out_dir"
	  mv $in_dir/RESELL*.zip $out_dir
  else
      logger -p local6.notice -s -t $(basename $0) "[WARN] No RESELL files found in $in_dir"
  fi
}

move_murs_to_processed (){
  in_dir=/shared/application_data/owi_084
  out_dir=/shared/application_data/moosemobile/unprocessed
  
  if [ -e $in_dir/RESELL_MUR* ]; then
      logger -p local6.notice -s -t $(basename $0) "[OK] RESELL files are moved from $in_dir to $out_dir"
	  mv $in_dir/RESELL_MUR* $out_dir
  else
      logger -p local6.notice -s -t $(basename $0) "[WARN] No RESELL files found in $in_dir"
  fi
}
...
usage() {
  cat <<EOF
 usage:
     $1 --put_owi | --put_logistics | --get_shipped | --put_cdrs
         [-h|--help]
...
EOF
 ...
 options=$(getopt -o hols -l help,put_owi,put_logistics,get_shipped -- "$@")
 ...
 do
     ...
     --put_cdrs)		move_cdrs_to_processed; break;;
	 --put_murs)		move_murs_to_processed; break;;
