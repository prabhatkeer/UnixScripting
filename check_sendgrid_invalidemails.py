...
##----------------------------------------------------------------------------------------------------
## This script uses the Sendgrid V2 API to gather the count for the invalid emails for the past one day
## https://sendgrid.com/docs/API_Reference/Web_API/invalid_emails.html
##
## CHANGE LOG:
##----------------------------------------------------------------------------------------------------
...
try:
    ...
    f = urllib.urlopen("https://api.sendgrid.com/api/invalidemails.count.json?%s" % params)
    response = f.read()
    json_data = json.loads(response)
    count_invalidemails = int(json_data['count'])

    if count_invalidemails >= 50:
        print "[CRITICAL] " + str(count_invalidemails) + " invalid emails since yesterday"
        exit(2)

    elif count_invalidemails >= 10:
        print "[WARNING] " + str(count_invalidemails) + " invalid emails since yesterday"
        exit(1)

    else:
        print "[OK] " + str(count_invalidemails) + " invalid emails since yesterday"
        exit(0)
