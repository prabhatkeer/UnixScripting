...
##----------------------------------------------------------------------------------------------------
## This script uses the Sendgrid V2 API to gather the count for the bounced emails for the past one day
...
try:
    ...
    f = urllib.urlopen("https://api.sendgrid.com/api/bounces.count.json?%s" % params)
    response = f.read()
    json_data = json.loads(response)
    count_bounces = int(json_data['count'])

    if count_bounces >= 50:
        print "[CRITICAL] " + str(count_bounces) + " bounced emails since yesterday"
        exit(2)

    elif count_bounces >= 10:
        print "[WARNING] " + str(count_bounces) + " bounced emails since yesterday"
        exit(1)

    else:
        print "[OK] " + str(count_bounces) + " bounced emails since yesterday"
        exit(0)
